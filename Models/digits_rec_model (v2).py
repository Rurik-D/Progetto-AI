...
'''
If the path of filename is included in parameters, the function
will simply return the name of that path.
Conversely, if is not included any path, the function will
open a window to select manually the file that contains the dataset.
'''
...
'''
It select the most suitable device, choosen among CPU, GPU, MPS.
If feedback is set on True, the device in use will be displayed.
'''
...
'''
Inherits the torch.Dataset structure and allows to customize it
in order to conform the QMNIST Dataset to the classification purpose.
'''
